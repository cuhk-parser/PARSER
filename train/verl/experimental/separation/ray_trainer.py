...
from verl.trainer.ppo.metric_utils import (
    ...
    compute_grpo_group_solve_metrics,
    compute_reward_extra_log_metrics,
    compute_throughout_metrics,
    compute_timing_metrics,
    compute_variance_proxy_metrics,
    grpo_group_solve_counts,
)
...
class SeparateRayPPOTrainer(RayPPOTrainer):
    ...
    def _fit_collect_metrics(self, batch):
        ...
        grpo_metrics = compute_grpo_group_solve_metrics(batch)
        counts = grpo_group_solve_counts(batch)
        if counts is not None and getattr(batch, "meta_info", None):
            meta = batch.meta_info
            d_all = int(meta.get("fully_async/dynamic_sampling/dropped_solve_all_groups", 0) or 0)
            d_none = int(meta.get("fully_async/dynamic_sampling/dropped_solve_none_groups", 0) or 0)
            if d_all or d_none:
                n_g, n_a, n_n = counts
                denom = n_g + d_all + d_none
                if denom > 0:
                    for k in list(grpo_metrics.keys()):
                        if k.endswith("/solve_all"):
                            grpo_metrics[k] = float(n_a + d_all) / denom
                        elif k.endswith("/solve_none"):
                            grpo_metrics[k] = float(n_n + d_none) / denom
        metrics.update(grpo_metrics)
        if self.reward_extra_infos_dict:
            metrics.update(compute_reward_extra_log_metrics(self.reward_extra_infos_dict))
