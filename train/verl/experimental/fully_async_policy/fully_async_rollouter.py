...
class FullyAsyncRollouter(SeparateRayPPOTrainer):
    ...
    async def set_max_required_samples(self):
        async with self.lock:
            ...
            self.max_concurrent_samples = len(self.async_rollout_manager.server_handles) * 25
    ...
    async def _should_pause_generation(self) -> bool:
        ...
        if self.staleness_samples >= self.max_required_samples:
            # Counter tracks pending_queue dequeues, not MQ consumption. If the trainer has
            # drained the message queue while not yet reaching reset_staleness (param sync),
            # we can get staleness_samples pinned at the cap with mq=0 and no in-flight
            # rollouts — rollouter stays paused and trainer blocks forever. Resync to zero.
            async with self.lock:
                inflight = len(self.active_tasks)
                if queue_size == 0 and inflight == 0:
                    print(
                        "[FullyAsyncRollouter][ShouldPause] "
                        f"staleness cap reached ({self.staleness_samples}/{self.max_required_samples}) "
                        "but mq empty and no active rollouts; resetting staleness_samples to avoid deadlock."
                    )
                    self.staleness_samples = 0
                    return False
            if not self.paused:
