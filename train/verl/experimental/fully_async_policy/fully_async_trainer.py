...
class FullyAsyncTrainer(SeparateRayPPOTrainer):
    ...
    def _dynamic_sampling_enabled(self) -> bool:
        dynamic_sampling_config = self.config.async_training.get("dynamic_sampling", {})
        if isinstance(dynamic_sampling_config, bool):
            return dynamic_sampling_config
        return bool(dynamic_sampling_config.get("enable", False))

    def _dynamic_sampling_eps(self) -> float:
        dynamic_sampling_config = self.config.async_training.get("dynamic_sampling", {})
        if isinstance(dynamic_sampling_config, bool):
            return 1e-6
        return float(dynamic_sampling_config.get("eps", 1e-6))

    @staticmethod
    def _extract_rollout_rewards(rollout_sample: Any) -> list[float] | None:
        full_batch = getattr(rollout_sample, "full_batch", None)
        if full_batch is None or "rm_scores" not in full_batch.batch:
            return None

        rm_scores = full_batch.batch["rm_scores"]
        if rm_scores.ndim == 1:
            rewards = rm_scores
        else:
            # `rm_scores` may be either sparse terminal rewards or dense token-level rewards.
            # For dynamic sampling we need one scalar per rollout; max keeps binary rewards at 0/1.
            rewards = rm_scores.max(dim=-1).values
        return [float(x) for x in rewards.detach().cpu().tolist()]

    def _dynamic_sample_drop_kind(self, rollout_sample: Any) -> str | None:
        """If this sample would be dropped by dynamic sampling, return ``\"all_zero\"`` or ``\"all_one\"``."""
        rewards = self._extract_rollout_rewards(rollout_sample)
        if not rewards:
            return None

        eps = self._dynamic_sampling_eps()
        all_zero = all(abs(reward) <= eps for reward in rewards)
        all_one = all(abs(reward - 1.0) <= eps for reward in rewards)
        if all_zero:
            return "all_zero"
        if all_one:
            return "all_one"
    ...
    async def _get_samples_from_queue(self) -> tuple[None, None] | tuple[int, Any]:
        ...
        queue_samples = []
        queue_len = 0
        dropped_dynamic_samples = 0
        dropped_solve_all_groups = 0
        dropped_solve_none_groups = 0
        dynamic_sampling_enabled = self._dynamic_sampling_enabled()
        while len(queue_samples) < self.required_samples:
            ...
            drop_kind = self._dynamic_sample_drop_kind(rollout_sample) if dynamic_sampling_enabled else None
            if drop_kind is not None:
                dropped_dynamic_samples += 1
                if drop_kind == "all_one":
                    dropped_solve_all_groups += 1
                else:
                    dropped_solve_none_groups += 1
                if dropped_dynamic_samples % 64 == 0:
                    print(
                        f"[FullyAsyncTrainer][DynamicSampling] Dropped {dropped_dynamic_samples} samples with "
                        f"all-zero/all-one rewards. Accepted {len(queue_samples)}/{self.required_samples}. "
                        f"mq_len: {queue_len}"
                    )
                continue
        ...
        print(
            ...
            f"mq_len: {queue_len}, "
            f"dropped_dynamic_samples: {dropped_dynamic_samples}"
        )
        ...
        batch.meta_info["fully_async/dynamic_sampling/dropped_samples"] = dropped_dynamic_samples
        batch.meta_info["fully_async/dynamic_sampling/dropped_solve_all_groups"] = dropped_solve_all_groups
        batch.meta_info["fully_async/dynamic_sampling/dropped_solve_none_groups"] = dropped_solve_none_groups
        return 0, batch
