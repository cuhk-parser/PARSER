...
def grpo_group_solve_counts(batch: DataProto) -> tuple[int, int, int] | None:
    """Return ``(n_groups, n_solve_all, n_solve_none)`` for GRPO ``uid`` groups, or ``None`` if unavailable."""
    uids = batch.non_tensor_batch.get("uid")
    if uids is None:
        return None
    if "token_level_scores" in batch.batch.keys():
        seq = batch.batch["token_level_scores"].sum(-1).detach().float().cpu().numpy()
    elif "token_level_rewards" in batch.batch.keys():
        seq = batch.batch["token_level_rewards"].sum(-1).detach().float().cpu().numpy()
    else:
        return None

    uid_to_rs: dict[str, list[float]] = defaultdict(list)
    for i in range(len(uids)):
        uid_to_rs[str(uids[i])].append(float(seq[i]))

    n_groups = len(uid_to_rs)
    if n_groups == 0:
        return None

    n_all = sum(1 for rs in uid_to_rs.values() if rs and all(r > 0 for r in rs))
    n_none = sum(1 for rs in uid_to_rs.values() if rs and all(r <= 0 for r in rs))
    return n_groups, n_all, n_none


def compute_grpo_group_solve_metrics(batch: DataProto, prefix: str = "reward_extra") -> dict[str, float]:
    """Fraction of prompts (``uid`` groups) where all rollouts are correct vs all incorrect.

    A rollout is treated as correct if its summed sequence score/reward is strictly positive.
    Groups with mixed outcomes are excluded from both ``solve_all`` and ``solve_none``.
    """
    counts = grpo_group_solve_counts(batch)
    if counts is None:
        return {}
    n_groups, n_all, n_none = counts
    return {
        f"{prefix}/solve_all": float(n_all) / n_groups,
        f"{prefix}/solve_none": float(n_none) / n_groups,
    }


_REWARD_EXTRA_FLATTEN_KEYS = frozenset({"tool_calls_per_turn", "subagent_replies_per_query"})


def _flatten_reward_extra_list_values(values: Any) -> np.ndarray:
    """Flatten per-trajectory event lists for batch-level stats."""
    nums: list[float] = []
    for value in np.asarray(values, dtype=object).ravel():
        if isinstance(value, (list, tuple, np.ndarray)):
            nums.extend(float(x) for x in value)
    if not nums:
...
def compute_reward_extra_log_metrics(
    reward_extra_infos_dict: dict[str, Any],
    prefix: str = "reward_extra",
) -> dict[str, float]:
    """Reduce per-sample ``reward_extra_info`` arrays to scalars for loggers (e.g. wandb)."""
    out: dict[str, float] = {}
    ...
    for key, values in reward_extra_infos_dict.items():
        ...
        if flat.size == 0:
            continue
        if key == "prediction":
            texts = [str(x) for x in flat]
            lens = [len(t) for t in texts]
            out[f"{prefix}/prediction/char_len_mean"] = float(np.mean(lens))
            nonempty = sum(1 for t in texts if t.strip() and t.strip() != "No answer found")
            out[f"{prefix}/prediction/nonempty_frac"] = float(nonempty) / len(texts)
            continue
        if key in _REWARD_EXTRA_FLATTEN_KEYS:
            nums = _flatten_reward_extra_list_values(values)
        else:
            try:
                nums = flat.astype(np.float64)
        ...
        if nums.size == 0:
            continue
        out[f"{prefix}/{key}/mean"] = float(np.mean(nums))
        out[f"{prefix}/{key}/max"] = float(np.max(nums))
        out[f"{prefix}/{key}/min"] = float(np.min(nums))
    return out
