...
_megatron_engine_import_error: Optional[BaseException] = None


def note_megatron_engine_import_failed(exc: BaseException) -> None:
    """Remember why Megatron engines were not registered (usually a failed optional dependency import)."""
    global _megatron_engine_import_error
    _megatron_engine_import_error = exc
...
class EngineRegistry:
    ...
    def get_engine_cls(cls, model_type: str, backend: str):
        ...
        if backend not in cls._engines[model_type]:
            if backend == "megatron" and _megatron_engine_import_error is not None:
                raise RuntimeError(
                    "Megatron backend is not registered because importing verl.workers.engine.megatron failed: "
                    f"{_megatron_engine_import_error}. "
                    "Typical cause: FlashAttention or Transformer Engine binary mismatch with your PyTorch build "
                    "(e.g. undefined symbol in flash_attn_2_cuda*.so). "
                    "Reinstall flash-attn built for your exact torch and CUDA version."
                ) from _megatron_engine_import_error
