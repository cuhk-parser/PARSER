...
def fsdp2_sharded_save_to_cpu(
    ...
    for param_name, param in model.named_parameters():
        ...
        if not isinstance(param, DTensor):
            # Save non-sharded parameters as independent CPU snapshots.
            cpu_tensor = param.detach().cpu().clone()
        ...
        # 2. Move to CPU memory and detach from computation graph. Clone to avoid
        # aliasing CPU-offloaded parameter storage across subsequent updates.
        local_cpu_tensor = local_gpu_tensor.detach().cpu().clone()
