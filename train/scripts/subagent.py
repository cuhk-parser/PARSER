...
class Subagent:
    ...
    def __init__(
        ...
        self.base_url = base_url # deprecated, as we now use socket
        self.model = model
...
class SubagentManager:
    ...
    async def query_all(self, query: str) -> tuple[str, int]:
        if self.unix_socket_path:
            ...
            raise BaseException("unix_socket_path is not set")
            connector = aiohttp.TCPConnector(
                limit=self.connector_limit,
                enable_cleanup_closed=True,
                keepalive_timeout=3,
            ) # keepalive_timeout is set to 3 seconds to avoid sglang Uvicorn 的空闲长连接超时竞争
